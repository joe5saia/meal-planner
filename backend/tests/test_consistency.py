...
class TestRecipeDeletionConsistency:
    ...
    def test_delete_recipe_removes_from_box(self, client, recipe_in_box):
        ...
        assert recipe_id not in box_ids, (
            "Recipe should not appear in box after deletion"
        )
    ...
    def test_delete_recipe_removes_from_meal_plans(self, client, recipe_in_box):
        ...
        client.post(
            "/api/meal-plans",
            json={"recipe_id": recipe_id, "date": date1}
        )
        client.post(
            "/api/meal-plans",
            json={"recipe_id": recipe_id, "date": date2}
        )
        ...
        assert len(recipe_plans) == 0, (
            "All meal plan entries for the recipe should be deleted"
        )
...
class TestOrphanedDataDetection:
    ...
    def test_no_orphaned_box_entries(self, client, sample_recipe_data, test_db):
        ...
        box_entries_before = test_db.query(RecipeBoxEntry).filter(
            RecipeBoxEntry.recipe_id == recipe_id
        ).count()
        assert box_entries_before == 1
        
        client.delete(f"/api/recipes/{recipe_id}")
        
        orphaned = test_db.query(RecipeBoxEntry).filter(
            RecipeBoxEntry.recipe_id == recipe_id
        ).count()
        
        assert orphaned == 0, (
            f"Found {orphaned} orphaned RecipeBoxEntry records"
        )
    ...
    def test_no_orphaned_meal_plan_entries(self, client, sample_recipe_data, test_db):
        ...
        entries_before = test_db.query(MealPlanEntry).filter(
        ...
        orphaned = test_db.query(MealPlanEntry).filter(
            MealPlanEntry.recipe_id == recipe_id
        ).count()
        
        assert orphaned == 0, (
            f"Found {orphaned} orphaned MealPlanEntry records"
        )
