...
class TestNormalizeWithLLM:
    ...
    async def test_normalize_with_llm_returns_parsed_results(self):
        ...
        mock_response.choices[0].message.content = json.dumps({
            "ingredients": [
                {"original": "2 cups diced tomatoes", "canonical_name": "tomato", "category": "Produce"},
                {"original": "1 tbsp olive oil", "canonical_name": "olive oil", "category": "Grocery"},
            ]
        })
    ...
    async def test_normalize_with_llm_handles_array_response(self):
        ...
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = json.dumps([
            {"original": "ground cumin", "canonical_name": "cumin", "category": "Spices & Seasonings"},
        ])
...
class TestCaching:
    ...
    def test_save_to_cache_creates_entries(self, test_db):
        ...
        onion_cache = test_db.query(IngredientCache).filter(
            IngredientCache.original_text == "diced onions"
        ).first()
    ...
    def test_save_to_cache_skips_duplicates(self, test_db):
        ...
        cached = test_db.query(IngredientCache).filter(
            IngredientCache.original_text == "2 cups flour"
        ).all()
        assert len(cached) == 1
