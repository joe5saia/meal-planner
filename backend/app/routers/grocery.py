...
from ..services.ingredient_parser import is_section_header, parse_ingredient
...
def get_category(ingredient_name: str) -> str:
    ...
    if re.search(
        r"\bground\s+(cumin|cinnamon|nutmeg|ginger|clove|allspice|cardamom|coriander|turmeric|pepper)\b",
        lower,
    ):
...
async def aggregate_with_llm(db: Session, ingredients: list[str]) -> list[GroceryItem]:
    ...
    for i, parsed in enumerate(parsed_ingredients):
        ...
        grouped[key].append({"parsed": parsed, "original": original, "category": norm["category"]})
    ...
    for (canonical_name, unit, category), items in grouped.items():
        ...
        quantities = [
            item["parsed"]["quantity"] for item in items if item["parsed"]["quantity"] is not None
        ]
...
def aggregate_without_llm(ingredients: list[str]) -> list[GroceryItem]:
...
async def generate_grocery_list(request: GroceryListRequest, db: Session = Depends(get_db)):
    ...
    if request.recipe_ids:
        ...
        entries = (
            ...
            .filter(
                MealPlanEntry.date >= request.start_date, MealPlanEntry.date <= request.end_date
            )
            .all()
        )
        recipes = [entry.recipe for entry in entries if entry.recipe is not None]
    else:
        raise HTTPException(
            status_code=400, detail="Provide either recipe_ids or both start_date and end_date"
        )
